...
const FILL_GAP_AVAILABLE_METHODS = [
  'padding',
  'margin',
  'width',
  'max-width',
...
const fillGapTarget = ($target, isLockable = false) => {
  if (isElement($target)) {
    ...
    if (
      ...
      const $lockableParent = findParentBySelector(
        $target,
...
const unfillGapTarget = ($target) => {
  if (isElement($target)) {
    if ($target.getAttribute('data-scroll-lock-filled-gap') === 'true') {
      const currentFillGapMethod = $target.getAttribute(
        'data-scroll-lock-current-fill-gap-method'
      );
